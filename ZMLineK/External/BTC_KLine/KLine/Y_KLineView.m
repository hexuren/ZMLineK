...
- (void)setKLineModels:(NSArray *)kLineModels
{
    ...
    if (self.scrollView.contentSize.width - 47 >= SCREEN_WIDTH) {
...
- (void)event_longPressMethod:(UILongPressGestureRecognizer *)longPress {
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state){
        ...
        if(!self.verticalView){
            ...
            self.verticalView.backgroundColor = gTextColorSub;
        ...
        if(!self.horizonView){
            ...
            self.horizonView.backgroundColor = gTextColorSub;
        ...
        if(!self.dateLabel){
            ...
            [self.dateLabel setBackgroundColor:UIColorFromHexWithAlpha(0x333333, 0.8)];
            [self.scrollView addSubview:self.dateLabel];
            double top = self.scrollView.frame.size.height - 12.0;
        ...
        else if ((rightXPosition + 30) >= (self.scrollView.contentOffset.x + SCREEN_WIDTH) ) {
            datePositionX = self.scrollView.contentOffset.x + SCREEN_WIDTH - 70;
        }
        else {
        ...
        if(!self.priceLabel){
            ...
            [self.priceLabel setBackgroundColor:UIColorFromHexWithAlpha(0x333333, 0.8)];
        ...
        //更新价格位置
        CGFloat pricePositionX = self.scrollView.contentOffset.x + SCREEN_WIDTH - 60;
        [self.priceLabel mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(@(rightYPosition));
