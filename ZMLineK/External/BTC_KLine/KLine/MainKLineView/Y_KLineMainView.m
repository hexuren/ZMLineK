...
- (UIView *)currentPriceView {
    if (!_currentPriceView) {
        ...
        [_currentPriceView setBackgroundColor:gMainColor];
    }
...
- (void)drawKLineChart{
    ...
    if(self.MainViewType == Y_StockChartcenterViewTypeKline)
    {
        ...
        CAShapeLayer *timelayer = [CAShapeLayer getSingleLineLayerWithPointArray:positions lineColor:gMainColor];
...
- (void)drawBottomDateValue {
    ...
    [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.lineKTime == 0 || self.lineKTime == 4) {
        ...
        if(CGPointEqualToPoint(lastDrawDatePoint, CGPointZero) || (point.x - lastDrawDatePoint.x > 60) ){
            if (self.isFullScreen) {
                CGRect rect = CGRectMake(drawDatePoint.x + 2, SCREEN_WIDTH - 98, 50, self.frame.size.height -Y_StockChartKLineMainViewMaxY);
...
- (void)drawMaxAndMinValueAndCurrentPrice{
    ...
    CATextLayer *MintextLayer = [CATextLayer fl_getTextLayerWithString:isMinRight ? [NSString stringWithFormat:@"%@→",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:low.doubleValue  afterPoint:[self.needDrawKLineModels firstObject].price]] : [NSString stringWithFormat:@"←%@",[CommonMethod calculateWithRoundingMode:NSRoundPlain roundingValue:low.doubleValue  afterPoint:[self.needDrawKLineModels firstObject].price]] textColor:[UIColor assistTextColor] fontSize:9.f backgroundColor:[UIColor clearColor] frame:rectMin];
...
//        y = ABS(maxY - (model.Close.doubleValue - low.doubleValue)/unitValue);
    ...
    CGFloat screenWidth = self.isFullScreen ? SCREEN_HEIGHT :SCREEN_WIDTH;
    ...
    [self.currentPriceView setFrame:CGRectMake(priceRectx, y - 7.5, Y_StockChartKLinePriceViewWidth, 15)];
